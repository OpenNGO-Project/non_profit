...
- REQ-NP-SETUP-01: Install/migrate setup (`setup_non_profit` on `after_install`, `ensure_fundraising_fixtures` on `after_migrate`) creates/updates custom fields (`Customer.household`, `Contact.title`, `Task.major_gift`, `Task.donor_interaction`), Non Profit Settings defaults, and fundraising fixtures idempotently; only when Good Connector is installed on the current site does it ask the shared setup to create Donation QRR and EBICS fields. [Trace: `non_profit/setup.py`, `non_profit/non_profit/fundraising_setup.py`, `hooks.py`; Tests: `doctype/non_profit_settings/test_non_profit_settings.py`, `doctype/donation/test_bank_integration.py`]
