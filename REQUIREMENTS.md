...
donation data feeds `good_analytics`.
...
- REQ-NP-MEM-07: The combined Member+Membership creation endpoint accepts Contact-only, Customer-only, or Contact+Customer selections, creates/reuses the Member first, links the Contact to both Member and Customer when both are selected, then creates/reuses an open-ended Membership for the chosen Membership Type; the Member list opens this dialog via the native `listview_settings.primary_action` hook. [Trace: `doctype/member/member.py::create_member_and_membership` (whitelisted POST); Tests: `doctype/member/test_member.py`]
...
- REQ-NP-HH-01: Household groups Members and Donors who share a postal address into one solicitation unit via dated `Household Member` child rows (`link_doctype`, `link_name`, `from_date`, `to_date`, `is_primary`); a row without `to_date` is current, setting `to_date` turns it into history, and history rows are never deleted. Addresses and Contacts attach to a Household through standard Dynamic Links rendered on the form like on Member/Donor; `Customer.household` (Link → Household) and `Contact.title` ship as custom fields. [Trace: `non_profit/non_profit/doctype/household/`, `non_profit/setup.py::get_custom_fields`; Tests: `doctype/household/test_household.py`]
- REQ-NP-HH-02: Household validation requires `from_date`, rejects `to_date < from_date`, duplicate current rows for the same party, more than one current primary per Household, and a second current Household for a Member/Donor. [Trace: `doctype/household/household.py` validate; Tests: `doctype/household/test_household.py`]
- REQ-NP-HH-03: Concurrent Household mutations are serialized by locking affected Member/Donor rows in deterministic order before conflicting Household Member rows; ordinary derived-field reads during Member/Donor validation must not lock Household Member rows (no lock-order inversion). [Trace: `doctype/household/household.py`; Tests: `doctype/household/test_household.py`]
- REQ-NP-HH-04: Party links are synced from `Household.on_update` via `frappe.db.set_value` (never by saving the party doc, avoiding recursive validation), reconciling both saved and persisted prior rows so removal, retargeting, and deletion cannot leave stale state; `after_delete` performs the same cleanup, and the read-only `Member.household` / `Donor.household` derived fields are restored by their controllers on save. [Trace: `doctype/household/household.py`, `doctype/member/member.py`, `doctype/donor/donor.py`; Tests: `doctype/household/test_household.py`]
...
- REQ-NP-HH-06: `add_member_to_household(household, member, from_date, to_date=None, is_primary=False, *, ignore_permissions=False)` is the canonical service for presentation apps to append dated Member history; it requires write permission on both Household and Member unless an explicitly trusted caller passes `ignore_permissions=True`. [Trace: `doctype/household/household.py::add_member_to_household`; Tests: `doctype/household/test_household.py`]
- REQ-NP-HH-07: Household access is restricted to `Non Profit Manager`, matching Donor access, because a Household may expose both Member and Donor records. [Trace: `doctype/household/household.json` permissions; Tests: `doctype/household/test_household.py`]

### 2.3 Fundraising — Donors, Donations, Campaigns

- REQ-NP-FUN-01: `Donor.customer` is the canonical ERPNext Customer relation for donor identity; Donor stores no own email — donor email is read from `Donor.customer -> Customer.email_id` and copied onto Donation / Recurring Donation / Donation Receipt rows as an operational snapshot. `Donor.preferred_language` and `Donation Receipt.language` are Link fields to Frappe's `Language` DocType storing codes such as `de` or `en`. [Trace: `non_profit/non_profit/doctype/donor/donor.py`, `doctype/donor/donor.json`, `doctype/donation_receipt/donation_receipt.json`, `patches/backfill_donor_customers_from_email`; Tests: `doctype/donor/test_donor.py`, `doctype/donation_receipt/test_donation_receipt.py`]
- REQ-NP-FUN-02: `get_or_create_customer_for_donor()` reuses a Customer from a same-email Member first, then a same-email Customer, and otherwise creates a new Customer, linking Contact and Address rows to both Donor and Customer. [Trace: `doctype/donor/donor.py`; Tests: `doctype/donor/test_donor.py`]
...
- REQ-NP-RCP-03: A Donation Receipt may be saved empty, but submit requires at least one donation row and validates that every row is submitted, paid, inside the receipt period (normalizing Frappe Date values and Desk JSON string dates), belongs to the receipt Donor, and is not already linked to another active receipt. [Trace: `doctype/donation_receipt/donation_receipt.py` validate/on_submit; Tests: `doctype/donation_receipt/test_donation_receipt.py`]
...
- REQ-NP-REC-01: A daily scheduler job creates submitted, unpaid Donation rows for due active Recurring Donation schedules, advances `next_date`, and cancels schedules that pass `end_date`, committing or rolling back each schedule independently so one failing schedule does not undo the batch; the "Create Next Donation Now" action creates an installment immediately and also advances the schedule, and `Paused` stops generation without ending the instruction. [Trace: `doctype/recurring_donation/recurring_donation.py` (`process_recurring_donations`, whitelisted `create_next_donation`), `hooks.py scheduler_events`; Tests: none]
...
- REQ-NP-COM-03: Grant review invitations require write permission on the Grant Application and an Assessment Manager before the status is moved to `In Progress`; logged-in applicants use the `/my-grant` page and the `grant_application` web form. [Trace: `doctype/grant_application/grant_application.py`, `non_profit/non_profit/web_form/grant_application/`; Tests: `doctype/grant_application/test_grant_application.py`]
- REQ-NP-COM-04: Volunteer creation intentionally accepts Contact only, requires a Volunteer Type and an email on the Contact, and links the Contact to the Volunteer without creating or linking a Customer. [Trace: `doctype/volunteer/volunteer.py::create_volunteer_from_contact` (whitelisted POST); Tests: `doctype/volunteer/test_volunteer.py`]
...
- REQ-NP-DSK-01: The app ships the "Non Profit" Workspace and Workspace Sidebar as source fixtures (sidebar with `app: "non_profit"`, `module: "Non Profit"`, `standard: 1`) covering Good Help, fundraising, Major Gifts, membership, community, settings, and the Expiring Memberships report. [Trace: `non_profit/non_profit/workspace/non_profit/`, `non_profit/workspace_sidebar/non_profit.json`; Tests: none]
...
- REQ-NP-SETUP-01: Install/migrate setup (`setup_non_profit` on `after_install`, `ensure_fundraising_fixtures` on `after_migrate`) creates/updates custom fields (`Customer.household`, `Contact.title`, `Task.major_gift`, `Task.donor_interaction`), Non Profit Settings defaults, and fundraising fixtures idempotently. [Trace: `non_profit/setup.py`, `non_profit/non_profit/fundraising_setup.py`, `hooks.py`; Tests: `doctype/non_profit_settings/test_non_profit_settings.py`]
...
- REQ-NP-SETUP-03: Migrate patches remove India-specific PAN/80G data (legacy Donor/Member PAN custom fields, 80G certificate DocTypes and tables), remove the unused Certification module (DocTypes, tables, web forms), remove `Membership.company`, backfill donor Customers from legacy emails (post-model-sync), backfill major-gift donor roll-ups, and convert free-text next actions into Tasks. [Trace: `non_profit/patches.txt`, `non_profit/patches/`; Tests: covered indirectly by app test suite]
...
- REQ-NP-SEC-05: Identity creation helpers reject conflicting Contact+Customer selections instead of silently moving a Contact to another Donor/Member, and ambiguous guest-facing donor identities are sent to staff review (`reject` policy) rather than merged arbitrarily. [Trace: `doctype/donor/donor.py`, `doctype/member/member.py`, `non_profit/donor_identity.py`; Tests: `doctype/donor/test_donor.py`, `doctype/member/test_member.py`]
...
- **Two Swiss QR-bill engines on this bench, intentionally.** non_profit's
  ...
  independent, while its optional Good Connector imports are limited to shared
  integration services such as CAPTCHA and identity matching.
...
- **The Household child table IS the history.** Leaving a household means
  setting `to_date` on the row, never deleting it; `Member.household` /
  `Donor.household` are read-only derived fields that must only change through
  Household rows or the canonical `add_member_to_household` service (Household
  audit findings 2026-07; repo `AGENTS.md` "Household Model").
- **Household lock-order convention.** Mutation paths always acquire
  Member/Donor party locks before Household Member child-row locks; ordinary
  derived-field reads during party validation are deliberately non-locking to
  avoid lock-order inversion (final review fix, 2026-07).
...
- Code: `non_profit/hooks.py`, `non_profit/setup.py`, `non_profit/patches.txt`
  ...
  `next_actions.py`, `legacy_payments.py`, `swiss_qrbill.py`,
  ...
  tests tree (20 test modules incl. `non_profit/test_donate.py`).
