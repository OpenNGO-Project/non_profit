...
- REQ-NP-SETUP-01: Install/migrate setup (`setup_non_profit` on `after_install`, `non_profit.setup.after_migrate` on `after_migrate`) creates/updates the standard-master custom fields (`Customer.household`, `Contact.title`, hidden NPO identity fields on Contact/Customer/Supplier, and Task links), assigns them to the Non Profit module for ordered uninstall cleanup, repairs a declared Custom Field whose physical column is missing, and refreshes Non Profit Settings defaults and fundraising fixtures idempotently. Missing `Donation Receipt DE` and `Donation Slip CH` Print Formats are inserted; an existing body is upgraded only when it matches a known shipped-content hash, while operator-edited bodies are preserved. `Donation Thank You DE` remains create-only. Only when Good Connector is installed on the current site does setup ask the shared setup to create Donation QRR and EBICS fields. [Trace: `non_profit/setup.py`, `non_profit/non_profit/fundraising_setup.py`, `hooks.py`; Tests: `non_profit/non_profit/test_party_model_schema.py`, `test_fundraising_setup.py`, `doctype/non_profit_settings/test_non_profit_settings.py`, `doctype/donation/test_bank_integration.py`]
...
- REQ-NP-SEC-05: Identity creation helpers reject conflicting Contact+Customer selections instead of silently moving a Contact to another Donor/Member, reject Contacts explicitly classified as non-people from person roles/Households, and send ambiguous guest-facing donor identities to staff review (`reject` policy) rather than merging arbitrarily. [Trace: `doctype/donor/donor.py`, `doctype/member/member.py`, `doctype/volunteer/volunteer.py`, `doctype/household/household.py`, `non_profit/donor_identity.py`; Tests: role and Household test modules]
