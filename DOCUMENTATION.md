...
- **NPO Organization** is the canonical organization identity anchor. ERPNext Customer and Supplier remain operating/accounting parties linked through hidden preparatory identity fields. NPO Organization is a legal-identity grouping only and is never a ledger party: several operating Customers (for example a parent and its branch) may share one legal identity without being merged, and verified-identifier uniqueness belongs to the NPO Organization rather than to any operating Customer. `legal_form` and identifiers owned by other apps are source evidence, not verified identity; base `non_profit` ships no country-specific identifier normalizer. New records are autonamed `NPO-ORG-.#####.` — a naming series with its own `tabSeries` counter. The previous `format:NPO-ORG-{#####}` declaration resolved each braced parameter in isolation, so the counter prefix was built from nothing and the doctype drew from a single shared `tabSeries` row named `""` (adjacent numbers across unrelated doctypes, lock contention on one row). `setup.seed_naming_series_counters()` (run from the `[pre_model_sync]` patch of the same name) seeds the key from the highest name already issued, so the switch never re-issues an existing name. The seeding logic is a documented copy of `good_connector.naming_seed` (non_profit is the public repository and imports no connector code) — sanctioned twin group `naming-seed`, pinned by `good_connector.tests.test_naming_seed_twin_parity`.
