...
BENEDIKT_EMAIL = "benedikt@goodvanta.ge"


def send_donation_slip_smoke_to_benedikt() -> dict:
    """Manual smoke: send the non_profit Donation Slip CH QR PDF to Benedikt."""
    ...
    donor = frappe.get_doc(
        {
            "doctype": "Donor",
            "donor_name": "Benedikt Goodvantage",
            "donor_type": "QR Smoke Donor",
        }
    ).insert(ignore_permissions=True)
    get_or_create_customer_for_donor(donor, email=BENEDIKT_EMAIL)
    donor.reload()
    ...
    pdf_html = frappe.render_template(
        print_format.html or "", {"doc": donation, "frappe": frappe}
    )
    attachment = {
    ...
    queue_name = getattr(queue_doc, "name", None) or _latest_email_queue_for_recipient(
        BENEDIKT_EMAIL
    )
    if queue_name:
...
def _ensure_donor_type() -> None:
    if not frappe.db.exists("Donor Type", "QR Smoke Donor"):
        frappe.get_doc(
            {"doctype": "Donor Type", "donor_type": "QR Smoke Donor"}
        ).insert(ignore_permissions=True)
