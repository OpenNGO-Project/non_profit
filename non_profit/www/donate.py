...
def get_context(context):
    ...
    if frappe.request and frappe.request.method == "POST":
        ...
        frappe.local.flags.redirect_location = (
            f"/donate_confirm?donation={donation_name}"
        )
...
def _handle_submission(form):
    ...
    if campaign and not frappe.db.exists(
        "Donation Campaign", {"name": campaign, "status": "Active"}
    ):
...
def _verify_captcha(form) -> None:
    ...
    response = (
        form.get(GOODVANTAGE_CAPTCHA_RESPONSE_FIELD)
        or form.get("captcha_response")
        or ""
    ).strip()
