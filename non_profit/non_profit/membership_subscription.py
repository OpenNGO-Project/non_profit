...
def ensure_membership_subscription_plan(
    ...
    *,
    ...
    if mark_membership_type_as_subscription and membership_type_doc.meta.has_field(
        "is_subscription"
    ):
    ...
    if not item:
        frappe.throw(
            _("Please set a Linked Item for Membership Type {0}.").format(
                membership_type_doc.name
            )
        )
    ...
    if frappe.db.exists("Subscription Plan", plan_name):
        current = frappe.db.get_value(
            "Subscription Plan", plan_name, list(values), as_dict=True
        )
        updates = {
        ...
        if updates:
            frappe.db.set_value(
                "Subscription Plan", plan_name, updates, update_modified=False
            )
        return plan_name
...
def ensure_membership_subscription(
    ...
    *,
    ...
    if membership.get("subscription") and frappe.db.exists(
...
def is_subscription_membership_type(membership_type: str) -> bool:
    ...
    if not frappe.get_meta("Membership Type").has_field("is_subscription"):
        return False
    return bool(
        frappe.db.get_value("Membership Type", membership_type, "is_subscription")
    )
...
def _clear_membership_to_date(membership) -> None:
    if membership.get("to_date"):
        frappe.db.set_value(
            "Membership", membership.name, "to_date", None, update_modified=False
        )
