...
def verify_signature(body, endpoint="Donation"):
    ...
    if not webhook_secret:
        frappe.throw(_("Webhook Secret not configured in Non Profit Settings"))

    key = bytes(webhook_secret, "utf-8")
    body_bytes = bytes(body, "utf-8") if isinstance(body, str) else body

    dig = hmac.new(key=key, msg=body_bytes, digestmod=hashlib.sha256)
    generated_signature = dig.hexdigest()
    result = hmac.compare_digest(generated_signature, signature)

    if not result:
        frappe.throw(
            _("{0} Signature Verification Failed").format(endpoint),
...
def capture_razorpay_donations(*args, **kwargs):
    ...
    try:
        ...
        log = frappe.log_error(
            message, _("Error creating donation entry for {0}").format(donor.name)
        )
        notify_failure(log)
        return {"status": "Failed", "reason": e}
...
def notify_failure(log):
    try:
        ...
        sendmail_to_system_managers(
            _("[Important] [ERPNext] Razorpay donation webhook failed, please check."),
            content,
        )
    except Exception:
        pass
